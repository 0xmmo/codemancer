  ...
  if (outputFilePaths.length > 0) {
    ...
    if (codeBlocks.length > 0) {
      ...
      for (const { language, codeBlock } of codeBlocks) {
        ...
        process.stdout.write(chalk.white("Identifying placeholders:"));
...
async function configureCommandLineArguments() {
  const args = await yargs(hideBin(process.argv)).option("p", {
    alias: "prompt",
    type: "string",
...
async function writeCodeBlockToFile(newPath, codeBlock, verbosity) {
  try {
    ...
    fs.writeFileSync(newPath2, codeBlock, "utf-8");
    if (verbosity > 0) {
      console.log(chalk.white(`Code block written to ${newPath2}`));
    }
  }
}
